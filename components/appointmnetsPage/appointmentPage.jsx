...
const allFetcher = (ref_code) =>
	fetch(`${_url}/appointments?ref_code=${ref_code}`).then((res) => res.json());
...
const AppointmentPage = () => {
	...
	const { data: appointmentDetails, error: appointmentFetcherError } = useSWR(
		id,
		appointmentFetcher,
		{
	...
	return (
		<AppointmentsPageWrap>
			<FindCard appointments={appointments} handleChange={handleChange} fetcherError={allFetcherError} />
			<StatusCard appointment={appointment} appointmentDate={appointmentDate} fetcherError={appointmentFetcherError} />
		</AppointmentsPageWrap>
	);
};
